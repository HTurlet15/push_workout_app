...
export default function NextSetRow({ index, currentSet, nextSet, unit = 'kg', displayWeight, weightFadeAnim, onUpdateNextSet }) {
  ...
  const weightDelta = getDelta(nextWeight.edited ? displayWeightValue : null, displayCurrentWeightValue);
  const repsDelta = getDelta(nextReps.edited ? nextReps.value : null, currentSet.reps.value);
